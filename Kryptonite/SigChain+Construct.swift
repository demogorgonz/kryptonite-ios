...
enum SigChainLink {
    ...
    var string:String {
        ...
        case .invite(let teamInvite):
            ...
            return "\(SigChainLink.scheme)\(path)/" + teamInvite.path.joined(separator: "/")
        }
    }
}
...
extension TeamIdentity {
    ...
    func invitationBlock() throws -> (inviteURL:String, request:SigChain.Request){
        ...
        // create the url link
        let teamInvite = TeamInvite(initialTeamPublicKey: initialTeamPublicKey, blockHash: newBlockHash, seed: nonceSeed)
        let inviteLink = SigChainLink.invite(teamInvite).string
